        ...
        if (validateStep2()) {
            startTransition(async () => {
                try {
                    ...
                    setMessage(response ? "Inscription réussie !" : "Erreur lors de la création du compte");
                } catch (error) {
                    setMessage(`${error}`);
                }
            });
                        ...
                        <div className="flex flex-col gap-4 mb-5">
                           <p className="text-center bg-red-400">{message}</p>
                            ...
                            <div className="flex gap-4">
                                <button onClick={prevStep} className="px-4 py-2 rounded bg-white text-deepBlue">Précédent</button>
                                <button onClick={handleSubmit} disabled={isPending} className={`px-4 py-2 rounded bg-white text-deepBlue transition ${isPending ? 'opacity-50' : ''}`}>{isPending ? 'Inscription en cours...' : 'Soumettre'}</button>
                            </div>
                        </div>
                    )}
